...
def main():
    ...
    for epoch in range(n_epochs): # number of times to loop over the dataset
        ...
        print('Epoch %d loss: %.3f acc: %.3f val_loss: %.3f val_acc: %.3f val_rec: %.3f auc_score: %.4f'
                %(epoch+1, epoch_training_loss, epoch_training_accuracy, epoch_val_loss, epoch_val_accuracy, epoch_val_recall, epoch_val_auc_score ))
